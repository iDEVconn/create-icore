...
- **Env layering (3 layers):**
  - **Transport wiring** (gateway ↔ MS): `${PREFIX}_TRANSPORT` (`tcp` default | `redis` | `nats` | `mqtt` | `rmq` | `kafka`) + the matching host/port/url vars; read by `buildTransport(prefix)` in `libs/shared/src/transport.ts`. All six are message-pattern transports (work with `@MessagePattern` + `ClientProxy.send/emit`). Each broker transport's driver (`nats`, `mqtt`, `amqplib`+`amqp-connection-manager`, `kafkajs`) is an optional peer dep of `@nestjs/microservices`; `rewriteRootPackageJson` adds it to the generated `package.json` for the chosen transport.
  - **gRPC is not wired into `buildTransport()` by design.** Three reasons: (1) `ClientProxy` vs `ClientGrpc` — all 6 transports return `ClientProxy` → `send(pattern, data)`; gRPC returns `ClientGrpc` → `getService<T>().method(req)`. Cannot unify in one `switch` without an adapter layer over both. (2) `.proto` files are contract-first — needs codegen pipeline wired into Nx before any logic. (3) No real gain in a monorepo — TCP socket between processes on the same host is equally fast; HTTP/2 + protobuf overhead only pays off cross-language or cross-network. **When gRPC makes sense:** services across multiple repos/languages/teams that need formal typed contracts. **If you want to add it:** (1) define `.proto` files per MS in `libs/proto/`; (2) run `ts-proto` codegen to generate typed clients; (3) add `@GrpcMethod('ServiceName', 'MethodName')` controllers alongside the existing `@MessagePattern` ones; (4) create `*GrpcClientModule` factories in the gateway injecting `ClientGrpc` instead of `ClientProxy`; (5) wire in `scaffold.ts` as a new transport option with its own env vars. Nothing blocks this — greenfield addition, not a migration.
