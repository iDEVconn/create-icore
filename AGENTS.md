...
- **Env layering (3 layers):**
  ...
  - **gRPC is not wired into `buildTransport()` by design.** gRPC uses a fundamentally different client model (`ClientGrpc` + `.proto` IDL + codegen, not `ClientProxy.send(pattern)`), so it cannot be added as a 7th case without a parallel abstraction layer. It is the right choice when: services span multiple repos/languages/teams and need formal typed contracts. For a single-language monorepo with one team, TCP is simpler and sufficient. **If you want to add gRPC:** (1) define `.proto` files per MS in `libs/proto/`; (2) run `ts-proto` codegen to generate typed clients; (3) add `@GrpcMethod('ServiceName', 'MethodName')` controllers alongside the existing `@MessagePattern` ones; (4) create `*GrpcClientModule` factories in the gateway that inject `ClientGrpc` instead of `ClientProxy`; (5) wire in `scaffold.ts` as a new transport option with its own env vars. Nothing in the current architecture blocks this — it is a greenfield addition, not a migration.
