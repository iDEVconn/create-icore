...
- **MANDATORY — TANSTACK ROUTE FILES ARE PROTECTED SOURCE:** `@tanstack/router-generator` (invoked via `tanstackRouter()` in every `apps/templates/client-*/vite.config.mts`) silently overwrites an _existing but empty_ route file with a default `Hello "<path>"!` scaffold on the next dev/build/test run that loads that Vite config. It cannot tell an intentionally-new empty file from a real implementation another process accidentally truncated — it just masks the loss. Never leave a route file under `src/routes/**` empty between edits (write the full replacement in one pass); if you find a route file already empty or containing that `Hello` scaffold, stop and flag it as corruption instead of letting the next `nx serve`/`nx build`/`nx test` silently regenerate it. CI runs `node tools/create-icore/scripts/check-route-integrity.mjs` (see `check` job in `pipeline.yml`) to catch empty routes, `Hello` scaffolds, and layout routes missing `<Outlet />` before this can happen unnoticed.
...
**AI usage tracking (opt-in, `@idevconn/ai-usage`):** `AiUsageService` (`apps/microservices/ai-orchestrator/src/app/ai-usage.service.ts`) records one row per LLM call into an `ai_usage_records` collection through the **same `DBStrategy` contract every other feature in this repo uses** — never a Supabase-specific client directly. Storage is wired via `ai-usage-db.provider.ts` (generated the same way as notes' `db.provider.ts`, `tools/create-icore/src/manifest/wire-ai-usage-db.ts`), but **independent of the notes demo's own db axis** — the `ai` feature can be chosen with `example=none`, so this file is always generated whenever `ai !== 'none'`, regardless of `example`. It provides the real `DBStrategy` for whatever `DB_PROVIDER` the project picked (supabase/firestore/mongodb/postgres — this repo's own committed default is supabase, migration: `supabase/migrations/20260908090835_create_ai_usage_records.sql`, generic `id text primary key, data jsonb not null` shape, same as every other Supabase-backed collection), or a `FakeDBStrategy`-backed `NullDbModule` (in-memory, lost on restart, never crashes) when `DB_PROVIDER=none`. `AiUsageService` never needs to know which one it got.

Since `LlmCallEvent` (llm-router's `onCall`/`onCost` payload) has no notion of _which_ gateway request or user triggered it — strategies are wrapped once at registry construction and shared across every call — `AiController` opens an `AsyncLocalStorage` context (`ai-usage.context.ts`) per `@MessagePattern` handler (`operation`/`userId`/`keySource`), and `app.module.ts`'s `onCall` hook reads it back to tag each recorded row; a call made outside any context is tagged `'unknown'` rather than silently dropped. `AiController` also exposes `ai.usage.summary`/`ai.usage.timeseries` message patterns. The gateway (`apps/api/src/app/ai/ai.module.ts`) mounts `AiUsageModule.forRoot()` from `@idevconn/ai-usage/server` — its `AdminAiUsageController` is registered by the package itself, so `@CheckAbility('read', 'AiUsage')` is applied **imperatively** (`CheckAbility(...)(AdminAiUsageController)`) rather than as a decorator, per the package's own README; `GatewayAiUsageDataSource` just forwards to the MS over `AiClientService.getUsageSummary`/`getUsageTimeseries` — the gateway never talks to any db provider directly. When `authProvider=none`, `create-icore` overlays an auth-none variant of `ai.module.ts` (`scaffold-auth-none.ts`'s `AI_MODULE_NO_AUTH_TS`) that drops the `CheckAbility` wiring entirely — `apps/api/src/app/abilities` doesn't exist in that mode (no login at all), so the dashboard is simply left ungated, same as every other route in an auth=none project. `by_user[].email` is `null` for every row (no `auth.getUser` RPC exists yet to enrich it) — a documented follow-up, not a bug. Client dashboard shipped for `client-shadcn` only (`/admin/ai-usage`, admin-only nav item + route, no chart library yet so the timeseries renders as a table) — `client-mui`/`client-antd` parity is a follow-up.
