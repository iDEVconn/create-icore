...
- **Open question, not resolved by this plan:** `tools/create-icore/_template-shell/package.json` and `tools/create-icore/templates/**/package.json` still pin `@idevconn/api-client` at `^0.3.0`/`^0.3.2` — older than the `^0.3.3` this repo's own root `package.json` and `libs/template-shared/package.json` already use. `templates/` is a generated build artifact (do not hand-edit); `_template-shell` is real source. Whether bumping `_template-shell`'s pin is in scope for this plan or a separate follow-up is a call for whoever executes this plan to make explicitly, not something to silently skip.
